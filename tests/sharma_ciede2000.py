"""Sharma et al. (2005) CIEDE2000 supplementary test data -- all 34 pairs.
...
Provenance
----------
The author's original host (www2.ece.rochester.edu / hajim.rochester.edu) was
unreachable from the build environment, so this table was reconstructed from
two independent published transcriptions and cross-checked row by row:

  * colour-science 0.4.7 -- colour/difference/tests/test_delta_e.py,
    ``test_delta_E_CIE2000_Sharma2004``  (33 rows; see note on pair 14)
  * coloraide 8.12.1 -- tests/test_distance.py, the block marked "# Sharma:"
    (34 rows)

All 33 rows the two sources share agree to within 1e-9 on every field.

Pair 14
-------
colour-science excludes pair 14 because ``np.arctan2`` rounds differently
across platforms at this input, so their table carries 33 rows; the excluded
values appear verbatim in a comment there. coloraide keeps pair 14 but
typos the sample a* as ``0.00010`` instead of ``0.0010``. The value used here
is ``0.0010``, which is what colour-science's comment records and what the
0.0009 / 0.0010 / 0.0011 / 0.0012 progression across pairs 13-16 requires.

Pair 14 is the exact-180-degree hue case and is a genuine floating-point
knife edge -- see ``test_color.py`` for what our implementation does with it.

Columns: (reference Lab, sample Lab, published dE2000).
"""
